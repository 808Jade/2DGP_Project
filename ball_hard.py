...
# ----------------------------------------Behavior-Tree----------------------------------------
    ...
    def print_strike_sign(self):
        ...
        strike_sign.sign_on()
        play_mode_hard.strike_counter.count += 1
        return BehaviorTree.SUCCESS
    ...
    def hit_action(self):  # 공을 때린 X 좌표의 위치에 따라 날아가는 방향 결정
        ...
        play_mode_hard.score_calculator.ball_x = self.arrive_x
        play_mode_hard.score_calculator.ball_y = self.arrive_y
        play_mode_hard.score_calculator.hit_x = swing_x
        play_mode_hard.score_calculator.hit_y = swing_y
        play_mode_hard.score_calculator.ball_size = self.size
        play_mode_hard.score_calculator.culculating()
        play_mode_hard.score_calculator.handle_total_score()
        print(play_mode_hard.score_calculator.result)
        print(play_mode_hard.score_calculator.total_score)
        self.hit_sign = True
