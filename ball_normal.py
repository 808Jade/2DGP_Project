...
class Ball_NORMAL:
    ...
    def __init__(self, x=440, y=380):
        ...
        if Ball_NORMAL.image is None:
            Ball_NORMAL.image = load_image('Ball.png')
...
# ----------------------------------------Behavior-Tree----------------------------------------
    ...
    def is_swing(self):
        if play_mode_normal.hitter.swing_x != 0:
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL
    ...
    def is_nice_swing_pos(self):
        if self.x - 20 < play_mode_normal.hitter.swing_mem_x < self.x + 20 \
                and self.y - 20 < play_mode_normal.hitter.swing_mem_y < self.y + 20:
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.FAIL
    ...
    def hit_action(self):  # 공을 때린 X 좌표의 위치에 따라 날아가는 방향 결정
        ...
        play_mode_normal.score_calculator.ball_x = self.arrive_x
        play_mode_normal.score_calculator.ball_y = self.arrive_y
        play_mode_normal.score_calculator.hit_x = swing_x
        play_mode_normal.score_calculator.hit_y = swing_y
        play_mode_normal.score_calculator.ball_size = self.size
        play_mode_normal.score_calculator.culculating()
        play_mode_normal.score_calculator.handle_total_score()
        print(play_mode_normal.score_calculator.result)
        print(play_mode_normal.score_calculator.total_score)
        self.hit_sign = True
